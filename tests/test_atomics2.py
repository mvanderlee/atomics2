"""End-to-end tests for the public atomics2 API against the bundled patomic lib."""
import sys
...
import atomics2
from atomics2 import Alignment, CmpxchgResult, MemoryOrder, OpType
from atomics2.exc import MemoryOrderError
...
@pytest.mark.parametrize("atype", [
    pytest.param(atomics2.INT, id="int"),
    pytest.param(atomics2.UINT, id="uint"),
])
...
def test_uint_wraparound() -> None:
    u = atomics2.atomic(width=4, atype=atomics2.UINT)
    u.dec()
...
def test_failed_cmpxchg_does_not_mutate_caller_bytes() -> None:
    ...
    b = atomics2.atomic(width=1, atype=atomics2.BYTES)
    b.store(b"\x07")
    expected = b"\x00"
    ...
    assert bytes([0])[0] == 0
    i = atomics2.atomic(width=1, atype=atomics2.UINT)
    i.store(9)
...
def test_bytes_store_load() -> None:
    b = atomics2.atomic(width=2, atype=atomics2.BYTES)
    b.store(b"\x0f\x00")
...
def test_view_shared_buffer() -> None:
    buf = bytearray(4)
    with atomics2.atomicview(buffer=buf, atype=atomics2.INT) as v:
        v.store(1234)
        assert v.load() == 1234
...
def test_readonly_view_supports_load_only() -> None:
    ...
    with atomics2.atomicview(buffer=ro, atype=atomics2.INT) as v:
        assert v.load() == 0
        assert v.readonly
...
def test_unsupported_width_raises() -> None:
    with pytest.raises(atomics2.exc.UnsupportedWidthException):
        Alignment(3000)
