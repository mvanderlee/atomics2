...
[`atomics2`](https://pypi.org/project/atomics2/). It updates the wrapped C library
...
Linux/MacOS:
```shell
$ python3 -m pip install atomics2
```
Windows:
```shell
$ py -m pip install atomics2
```
...
def fn(ai: atomics2.INTEGRAL, n: int) -> None:
    for _ in range(n):
        ai.inc()
...
if __name__ == "__main__":
    # setup
    a = atomics2.atomic(width=4, atype=atomics2.INT)
    total = 10_000
    # run threads to completion
...
def fn(shmem_name: str, width: int, n: int) -> None:
    ...
    buf = shmem.buf[:width]
    with atomics2.atomicview(buffer=buf, atype=atomics2.INT) as a:
        for _ in range(n):
            a.inc()
    del buf
    shmem.close()
...
### Types
The following helper (abstract-ish base) types are available in `atomics2`:
- [`ANY`, `INTEGRAL`, `BYTES`, `INT`, `UINT`]

This library provides the following `Atomic` classes in `atomics2.base`:
- `Atomic --- ANY`
...
This library also provides `Atomic*View` (in `atomics2.view`) and 
`Atomic*ViewContext` (in `atomics2.ctx`) counterparts to the `Atomic*` classes, 
...
buf = bytearray(2)
with atomics2.atomicview(buffer=buf, atype=atomics2.BYTES) as a:
...
buf = bytearray(4)
ctx = atomics2.atomicview(buffer=buf, atype=atomics2.INT)
...
`atomics2` to ensure that a given buffer meets these requirements.
```python
from atomics2 import Alignment
...
def atomic_mul(a: atomics2.INTEGRAL, operand: int):
    res = atomics2.CmpxchgResult(success=False, expected=a.load())
    while not res:
...
The `MemoryOrder` enum class is provided in `atomics2`, and the memory orders 
...
### Exceptions
The following exceptions are available in `atomics2.exc`:
- `AlignmentError`
- `MemoryOrderError`
...
then copies the shared library into `atomics2._clib`.
...
copy-paste the shared library file into `atomics2._clib` manually.
...
`atomics2` is a fork of [atomics](https://github.com/doodspav/atomics),
